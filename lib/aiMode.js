...
class SmartChargingPlanner {
	...
	async calculate() {
		try {
			...
			// Step 5: Calculate energy gap between cheap phase end and solar sufficiency
			// Uses historical hourly consumption if available, otherwise flat householdConsumptionKw
			const { energyGapKwh, dataSource } = this._calculateEnergyGap(cheapPhase.endHour, solarSufficiencyHour, consumptionAnalysis);
			...
			this._adapter.log.info(`SmartChargingPlanner: Target SoC=${targetSoc}% | ${recommendation}`);
		} catch (error) {
	...
	/**
	 * Control cycle: check if we should enable or disable grid charging in evcc.
	 *
	 * Logic:
	 *   IN cheap phase AND SoC < targetSoc → enable grid charging (batterygridchargelimit = high value)
	 *   SoC >= targetSoc OR outside cheap phase → disable grid charging (batterygridchargelimit = 0)
	 */
	async _runControlCycle() {
		try {
			...
			if (!this._cachedCheapPhase) {
				const cheapPhase = await this._getCheapPhaseFromPricing();
				if (!cheapPhase) return; // pricing states not ready yet
				this._cachedCheapPhase = cheapPhase;
			}
			...
			const shouldCharge = inCheapPhase && batterySoc !== null && batterySoc < this._cachedTargetSoc;
			...
			if (shouldCharge) {
				...
				if (success) {
					...
					this._adapter.log.info(
						`SmartChargingPlanner: Grid charging ENABLED ` +
						`(cheap phase active, SoC ${batterySoc?.toFixed(0)}% < target ${this._cachedTargetSoc}%)`
					);
				}
			} else {
	...
	async _getBatterySoc() {
		try {
			const evccInstance = this._config.evccInstance || 'evcc.0';
			const state = await this._adapter.getForeignStateAsync(`${evccInstance}.status.batterySoc`);
			return state && state.val !== null ? parseFloat(state.val) : null;
		} catch (error) {
	...
	/**
	 * Calculate the energy the battery must provide during the gap (cheapPhaseEnd → solarSufficiency).
	 * Uses historical hourly consumption averages when available, falls back to static config value.
	 * @returns {{ energyGapKwh: number, dataSource: string }}
	 */
	_calculateEnergyGap(startHour, endHour, consumptionAnalysis) {
		...
		if (consumptionAnalysis && consumptionAnalysis.hourly && consumptionAnalysis.overall.dataPoints > 0) {
			// Integrate historical consumption hour by hour over the gap
			let totalKwh = 0;
			...
			for (let h = startInt; h < endInt; h++) {
				...
				// What fraction of this hour falls inside the gap?
				const slotStart = Math.max(h, startHour);
				const slotEnd   = Math.min(h + 1, endHour);
				const fraction  = slotEnd - slotStart;
				totalKwh += (avgWatts / 1000) * fraction;
			}

			return { energyGapKwh: totalKwh, dataSource: 'historical data' };
		}

		// Fallback: flat static value
		return {
			energyGapKwh: gapHours * this._householdConsumptionKw,
			dataSource: `static fallback (${this._householdConsumptionKw} kW)`
		};
	}

	/**
	 * Find the first forecast hour where PV production covers household consumption.
	 * Uses historical hourly consumption per hour when available, otherwise pvSufficiencyThreshold.
	 */
	_findSolarSufficiencyTime(pvProduction, cheapPhaseEndHour, consumptionAnalysis) {
		const hasHistory = consumptionAnalysis &&
		...
		this._adapter.log.warn(
			`SmartChargingPlanner: Solar sufficiency not found in forecast window, ` +
