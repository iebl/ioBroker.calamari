...
${context.cheapPhases.length > 0
	...
	: 'No cheap electricity phases scheduled in the next 24 hours'}
...
class SmartChargingPlanner {
	...
	async calculate() {
		try {
			...
			// Step 2: Get weather forecast from brightsky and calculate PV production
			const weatherForecast = await this._weatherService.getForecast();
			const pvProduction = this._pvForecast.calculateProduction(weatherForecast);

			// Step 3: Find when PV production first exceeds household consumption threshold
			const solarSufficiencyHour = this._findSolarSufficiencyTime(pvProduction, cheapPhase.endHour);

			// Step 4: Calculate energy gap between cheap phase end and solar sufficiency
			const energyGapHours = Math.max(0, solarSufficiencyHour - cheapPhase.endHour);
			const energyGapKwh = energyGapHours * this._householdConsumptionKw;

			// Step 5: Calculate required battery SoC
			...
			// Step 6: Sum PV production during the gap period for info
			...
			const recommendation = energyGapHours > 0
				? `Charge battery to ${targetSoc}% during cheap phase (ends ${cheapPhaseEndStr}). ` +
				  ...
				  `(gap: ${energyGapHours.toFixed(1)}h, ${energyGapKwh.toFixed(1)} kWh needed from battery).`
				: `Solar is already sufficient when cheap phase ends at ${cheapPhaseEndStr}. ` +
				  `Minimum charge of ${this._batteryMinSoc}% is sufficient.`;
	...
	/**
	 * Find the first forecast hour where PV production exceeds the sufficiency threshold,
	 * searching only after the cheap phase ends.
	 */
	_findSolarSufficiencyTime(pvProduction, cheapPhaseEndHour) {
		for (const hour of pvProduction) {
			const hourDecimal = hour.timestamp.getHours() + hour.timestamp.getMinutes() / 60;
			if (hourDecimal >= cheapPhaseEndHour && hour.estimatedKwh >= this._pvSufficiencyThreshold) {
				return hourDecimal;
			}
		}
		const fallback = Math.max(cheapPhaseEndHour + 4, 12);
		this._adapter.log.warn(
			`SmartChargingPlanner: Solar sufficiency (>=${this._pvSufficiencyThreshold} kWh) ` +
			`not found in forecast window, using conservative fallback ${this._formatHour(fallback)}`
		);
		return fallback;
	}
