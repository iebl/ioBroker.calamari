...
class SmartChargingPlanner {
	...
	async calculate() {
		try {
			...
			// Step 8: Estimate charging start time based on current SoC and AC charge power
			const currentSoc = await this._getBatterySoc();
			const currentSocVal = currentSoc !== null ? currentSoc : 0;
			const socDeficit = Math.max(0, targetSoc - currentSocVal);
	...
	async _sendTelegramMessage({ cheapPhaseStart, cheapPhaseEnd, grossRate, targetSoc,
		currentSoc, chargingStartTime, chargingDurationMin,
		...
		try {
			...
			const lines = [
				...
				``,
				`🔋 *Akku jetzt:* ${currentSoc}%  →  *Ziel:* ${targetSoc}%`,
