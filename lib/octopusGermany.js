...
class TokenManager {
	 ...
	 * @param {number|null} [expiry] - Optional explicit expiry timestamp
	 */
...
class OctopusGermany {
	 ...
	 * @param {string} [email] - Email address for the Octopus Germany account (can be set later via initialize())
	 * @param {string} [password] - Password for the Octopus Germany account (can be set later via initialize())
	...
	constructor(adapter, email, password, options = {}) {
		...
		// Sicherstellung, dass log existiert
		if (!adapter.log) {
			// Alternative Logging-Methode (fallback to console)
			// @ts-ignore - Fallback logger compatible with adapter interface
			this.log = {
	 ...
	 */
	get _token() {
		return this._tokenManager ? this._tokenManager.token : null;
	}
	...
	async login() {
		if (!this._tokenManager) {
			this._adapter.log.error("Token manager not initialized");
			return false;
		}
	 ...
	 * @param {string|null} [cacheKey] - Key to invalidate, or null for all
	 */
